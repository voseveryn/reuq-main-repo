...
export default async function RootLayout(props: PageProps) {
  ...
  const { page, navigation, footer, product, contact } = await client.query({
    page: queryBuilder.get(
      "Page",
      byLocaleUrl(urlToMatch),
      PageFragment(locale)
    ),
    navigation: queryBuilder.get(
      "Navigation",
      byUniqueOne,
      NavigationFragment(locale)
    ),
    footer: queryBuilder.get("Footer", byUniqueOne, FooterFragment(locale)),
    product: queryBuilder.get(
      "Product",
      byLocaleUrl(urlToMatch),
      ProductFragment(locale)
    ),
